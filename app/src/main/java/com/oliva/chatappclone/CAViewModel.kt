...
//        onLogout()
    ...
    fun onAddChat(number: String) {
        if (number.isEmpty() or !number.isDigitsOnly())
            handleException(customeMessage = "Number must only contain digits")
        else {
                ...
                .addOnSuccessListener {
                    if (it.isEmpty) {
                        db.collection(COLLECTION_USER).whereEqualTo("number", number)
                            ...
                            .addOnSuccessListener {
                                if (it.isEmpty)
                                    handleException(customeMessage = "Cannot retrieve user with number $number")
                                else {
                        ...
                        handleException(customeMessage = "Chat already exists")
                    }
                }
        }
    }
